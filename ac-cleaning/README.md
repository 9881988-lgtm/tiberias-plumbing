...
- Цены:
  - мойка без разборки: `250-350 ₪`;
  - глубокая мойка с разборкой и дезинфекцией: `400-500 ₪`.
...
До подключения `SETTINGS.googleScriptUrl` форма показывает локальные демо-слоты: воскресенье-четверг `08:00`, `11:00`, `14:00`, `17:00`; пятница `08:00`. После подключения Apps Script эти слоты заменяются реальной доступностью из Google Calendar.
